...
function getLatestNinjaValuesFromJson() {
    ...
    const ninjaChaosValues = JSON.parse(readFileSync(loc).toString());
...
export function getTabFromPoeApi(options) {
    const { accountName, POESESSID, league, tabIndex, } = options;
    return axios
        .get(`${tabBaseUrl}?accountName=${accountName}&realm=pc&league=${league}&tabs=0&tabIndex=${tabIndex}`, { headers: { cookie: `POESESSID=${POESESSID}` } })
        .then(res => {
        return res.data;
    })
        .catch(err => err);
...
//# sourceMappingURL=poeApi.js.map